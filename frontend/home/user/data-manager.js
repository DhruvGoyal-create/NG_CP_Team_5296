...
function onTransactionEvent(eventType, callback) {
...
// Trigger event
function triggerTransactionEvent(eventType, data) {
    if (eventListeners[eventType]) {
...
function setupCrossTabSync() {
    ...
    window.addEventListener('storage', function(e) {
        if (e.key === 'transactions') {
            ...
            // Find what changed
            const changes = detectTransactionChanges(oldTransactions, newTransactions);
            
            // Update local transactions
            transactions = newTransactions;
            
            // Trigger appropriate events
            changes.added.forEach(transaction => {
                triggerTransactionEvent('transactionAdded', transaction);
            });
            
            changes.updated.forEach(transaction => {
                triggerTransactionEvent('transactionUpdated', transaction);
            });
            
            changes.deleted.forEach(transactionId => {
                triggerTransactionEvent('transactionDeleted', transactionId);
            });
        }
    });
}

// Detect what changed in transactions
function detectTransactionChanges(oldTransactions, newTransactions) {
    const changes = {
        added: [],
        updated: [],
        deleted: []
    ...
    // Find updated transactions
    newTransactions.forEach(newTransaction => {
        const oldTransaction = oldTransactions.find(t => t.id === newTransaction.id);
        if (oldTransaction && JSON.stringify(oldTransaction) !== JSON.stringify(newTransaction)) {
            changes.updated.push(newTransaction);
        }
    });
...
function initializeData() {
    ...
    triggerTransactionEvent('dataLoaded', transactions);
}
...
function addTransaction(transaction) {
    ...
    triggerTransactionEvent('transactionAdded', newTransaction);
...
function updateTransaction(id, updates) {
    ...
    if (index > -1) {
        ...
        // Trigger real-time event
        triggerTransactionEvent('transactionUpdated', transactions[index]);
...
function deleteTransaction(id) {
    ...
    if (index > -1) {
        ...
        // Trigger real-time event
        triggerTransactionEvent('transactionDeleted', id);
...
window.transactionManager = {
    ...
    importFromCSV,
    clearAllData,
    onTransactionEvent, // New: Register event listeners
    triggerTransactionEvent // New: Manual event triggering
};
